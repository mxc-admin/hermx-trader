...
[1] TradingView Pine strategy fires an alert (2h/4h close)
        ...
        │  header:      X-Webhook-Secret: <SHADOW_WEBHOOK_SECRET>
        ▼
...
This is the `skills/vps-deploy.md` flow, condensed:
...
| **Dashboard auth** | Optional token (`HERMX_DASH_AUTH` + `HERMX_DASH_AUTH_TOKEN`) via `X-Dashboard-Token`; typically off on a same-host loopback deploy since it isn't publicly reachable. |
...
- `hermx-control` skill; `skills/vps-deploy.md`; systemd units in `deploy/`.
