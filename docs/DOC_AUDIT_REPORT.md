...
4. **Publish a single authoritative env-var reference** (e.g. `docs/CONFIGURATION.md` or expand `setup/env.example`) covering all 57 `HERMX_*` vars with defaults. Add the 17 MISSING vars; **delete** the 10 stale ones (`HERMX_EXEC_*`, `HERMX_CRON_*`, `HERMX_ENFORCE_ALERT_SCHEMA`, `HERMX_ADVISOR_MIN_SCORE`, `HERMX_MONITOR_SUMMARY_HOUR_UTC`, `HERMX_PROACTIVE_ENABLED`, `HERMX_SKILLS`) or reconcile them with intended behavior.
