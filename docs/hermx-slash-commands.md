...
| `/hx-trace` | read | Follow one signal intake→dedupe→pipeline→exec, joined on `received_at` | Never re-derives a time-less `signal_id`; read-only |
| `/hx-tv-alerts <name-or-id>` | read | Prints copy-paste BUY + SELL TradingView Message templates for one strategy | Read-only text; hard-codes venue/timeframe so schema passes; never sends or `/webhook` |
| `/hx-strategy-mode <name-or-id> <pause\|resume\|demo\|live>` | mutate | Sets a per-strategy override via dashboard control endpoint | Dry-run first; `live` needs explicit `yes`; never edits `strategies/*.json` |
| `/hx-close <symbol\|strategy>` | mutate | Flattens ONE position, reduce-only, via `/api/close` | UNKNOWN read → refuses; no size sent; never `/webhook` |
...
### `/hx-help` — command help (read-only text)
- **Syntax:** `/hx-help` or `/hx-help <command>`
...
- **Example:** `rtk claude -p "/hx-help close" --permission-mode dontAsk`

### `/hx-status` — system posture
- **Syntax:** `/hx-status`
...
- **Example:** `rtk claude -p "/hx-status" --permission-mode dontAsk`

### `/hx-positions` — open positions
- **Syntax:** `/hx-positions`
...
- **Example:** `rtk claude -p "/hx-positions" --permission-mode dontAsk`

### `/hx-strategy-list` — strategies & effective modes
- **Syntax:** `/hx-strategy-list`
...
- **Example:** `rtk claude -p "/hx-strategy-list" --permission-mode dontAsk`

### `/hx-trace` — follow one signal
...
- **Example:** `rtk claude -p "/hx-trace BTCUSDT" --permission-mode dontAsk`

### `/hx-tv-alerts` — copy-paste TradingView alert templates (read-only)
...
- **Example:** `rtk claude -p "/hx-tv-alerts SOLUSDT" --permission-mode dontAsk`

### `/hx-strategy-mode` — change a strategy's mode (mutating)
- **Syntax:** `/hx-strategy-mode <name-or-id> <pause|resume|demo|live>` (`resume` → wire `clear`)
...
- **Example:** `rtk claude -p "/hx-strategy-mode btcusdt_duo_base_dev_2h demo" --permission-mode dontAsk`

### `/hx-close` — flatten one position (mutating, reduce-only)
...
- **Example:** `rtk claude -p "/hx-close BTCUSDT" --permission-mode dontAsk`

### `/hx-emergency-stop` — layered stop (mutating)
...
- **Does:**
  ...
  - `demo <id>` — forces one strategy to sandbox via the control override (mutating twin of `/hx-strategy-mode <id> demo`).
...
- **Example:** `rtk claude -p "/hx-emergency-stop flatten" --permission-mode dontAsk`

### `/hx-restart` — restart the dashboard / receiver (mutating, lifecycle)
...
- **Example:** `rtk claude -p "/hx-restart" --permission-mode dontAsk`

### `/hx-upgrade` — upgrade the host to latest code (mutating, deploy + lifecycle)
- **Syntax:** `/hx-upgrade` or `/hx-upgrade --no-pull` or `/hx-upgrade --no-tests` or `/hx-upgrade --no-ui`
...
- **`/hx-close` is reduce-only and sizeless.** Body carries no size; the receiver derives the reduce-only close from the live position; it never routes via `/webhook`.
