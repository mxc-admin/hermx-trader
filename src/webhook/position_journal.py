...
so wr.apply_effect stays importable and monkeypatchable). To avoid a load-time
circular import, the shared decimal/stat/label helpers that still live in
webhook_receiver (D, dec_usd, dec_pct, empty_policy_stats, POLICY_LABELS and the
_USD_KEYS/_PCT_KEYS/_UNITS_KEYS sets) are resolved lazily via _wr() — a
sys.modules lookup performed at call time, not at import time. This also means
each per-test importlib.reload(webhook_receiver) is tracked automatically, since
_wr() always returns the current module object bound to the active SHADOW_ROOT.
"""
...
def _wr():
    """Late-bound handle to the webhook_receiver module.

    Resolved from sys.modules at call time (NOT import time) so this module can be
    imported by webhook_receiver without a circular import, and so each per-test
    importlib.reload(webhook_receiver) is picked up automatically.
    """
    return sys.modules["webhook_receiver"]


def refresh_compound_stats(ps: dict) -> None:
    wr = _wr()
    D, dec_usd, dec_pct = wr.D, wr.dec_usd, wr.dec_pct
    ...
    stats = ps.setdefault("stats", wr.empty_policy_stats())
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            ...
            if not isinstance(item, str):
                ...
                continue
            looks_numeric = (
                key_text in wr._USD_KEYS
                or key_text in wr._PCT_KEYS
                or key_text in wr._UNITS_KEYS
...
def _ensure_policy_bucket(state: dict, account_key: str, policy_key: str) -> dict:
    ...
    ps = policies_state.setdefault(policy_key, {"label": wr.POLICY_LABELS.get(policy_key, policy_key), "symbols": {}, "stats": wr.empty_policy_stats()})
    ps.setdefault("stats", wr.empty_policy_stats())
