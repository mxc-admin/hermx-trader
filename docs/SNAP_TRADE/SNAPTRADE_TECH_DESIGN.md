...
**Author:** Engineering
**Date:** 2026-07-03
**Prereq reading:** [`SNAPTRADE_RESEARCH.md`](./SNAPTRADE_RESEARCH.md) (feasibility, pricing, go/no-go) and [`SNAPTRADE_API_FINDINGS.md`](./SNAPTRADE_API_FINDINGS.md) (grounded API contract — real endpoint paths, field names, idempotency, webhooks, sandbox). This document is the *implementation blueprint* — the how, not the whether. Where the research doc concluded "GO for read-only aggregation, conditional/NO-GO for live execution," this doc designs both paths so the write path can be built behind gates and left dark until its pre-conditions clear.
